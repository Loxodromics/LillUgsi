...
///
/// Ownership model:
/// - CommandBufferManager owns all command pools it creates
/// - Clients receive raw VkCommandPool handles but don't own them
/// - CommandBufferManager is responsible for cleanup of all pools
...
public:
	...
	/// @return Raw handle to the created command pool (not owned by caller)
	[[nodiscard]] VkCommandPool createCommandPool(
		uint32_t queueFamilyIndex,
		VkCommandPoolCreateFlags flags = 0);

	/// Allocate command buffers from a command pool
	/// @param commandPool The pool to allocate buffers from (must be created by this manager)
...
private:
	...
	/// We keep full ownership of pools through these RAII handles
	std::vector<VulkanCommandPoolHandle> commandPools;

	/// Map to track which raw pool handle corresponds to which internal handle
	/// This allows us to validate that pools being used were created by this manager
	std::unordered_map<VkCommandPool, size_t> poolIndices;
	...
	/// Validate that a command pool was created by this manager
	/// @param commandPool The command pool to check
	/// @throws VulkanException if the pool was not created by this manager
	void validateCommandPool(VkCommandPool commandPool) const;
};
