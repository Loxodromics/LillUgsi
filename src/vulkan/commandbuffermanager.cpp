...
void CommandBufferManager::cleanup() {
	...
	for (auto& [pool, buffers] : this->allocatedCommandBuffers) {
		if (!buffers.empty()) {
			spdlog::trace("Freeing {} command buffers from pool {}",
...
VkCommandPool CommandBufferManager::createCommandPool(
	...
	/// Store the pool handle for tracking
	/// We use the index in our vector to reference this pool later
	this->commandPools.push_back(std::move(poolHandle));
	this->poolIndices[commandPool] = this->commandPools.size() - 1;
	...
	/// Return the raw handle for the client to use
	/// The manager maintains ownership of the actual resource
	return commandPool;
}
...
std::vector<VkCommandBuffer> CommandBufferManager::allocateCommandBuffers(
	...
	/// Verify this pool was created by our manager
	this->validateCommandPool(commandPool);
...
void CommandBufferManager::resetCommandPool(
	...
	/// When we reset a pool, we can forget tracking of command buffers
	/// since they all become available for reuse from the pool
	this->allocatedCommandBuffers[commandPool].clear();
...
void CommandBufferManager::validateCommandPool(VkCommandPool commandPool) const {
	/// Check if the pool exists in our tracking map
	/// This ensures the client is only using pools created by this manager
	if (this->poolIndices.find(commandPool) == this->poolIndices.end()) {
		throw VulkanException(
			VK_ERROR_VALIDATION_FAILED_EXT,
			"Command pool was not created by this manager",
			__FUNCTION__, __FILE__, __LINE__);
	}
}
